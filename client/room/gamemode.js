...
function Лидеры(id) {
    ...
    // Display the current Moscow time to the player
    player.PopUp("<color=yellow> 1. OZI</a> (7 ОП)            <color=grey>2. самоуб****во (6 ОП)</a>            <color=brown>3. Бухалкер (5 ОП)</a>");
} 
function Кубик(id) {
    let player = API.Players.GetByRoomId(parseInt(id));
...
function Help(id) {
	...
	p.PopUp('<b><i>Остальные команды: /Адм(rid, /Бан(rid), /Полет(rid), /Нхп("rid","количество хп"), /ЛКопобка(rid), /БКоробка(rid), /Статус("rid","статус")');
    }
}
function Полет(id) {
    let p = API.Players.GetByRoomId(parseInt(id));
